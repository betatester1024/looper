    ...
    script.onload = () => {
      WebFont.load({
        google: {
          families: [
            "Noto Sans Mono",
            "Raleway:100,200,400,500,600,700,900",
            "Noto Sans Display",
            "Noto Color Emoji"
          ]
        }
      ...
      send(
        ...
        (res2) => {
          ...
          if (link != "/server" && res2.status != "SUCCESS")
            ...
            resetExpiry(res2);
            ele.innerHTML = `Welcome, <kbd>${res2.data.user}</kbd> |
...
function minimalTime(ms, inFuture = false) {
  ...
  if (ms < 6e4) return inFuture ? "shortly" : "just now";
  let day = Math.floor(ms / 1e3 / 60 / 60 / 24);
  ms = ms % (1e3 * 60 * 60 * 24);
  let hr = Math.floor(ms / 1e3 / 60 / 60);
  ms = ms % (1e3 * 60 * 60);
...
function login_v2(ev, signup = false) {
  ...
  if (loginDialog && loginDialog.isOpen) return;
...
function bSelInitialise() {
  ...
  document.addEventListener("pointerup", (e) => {
    if (!e.target || e.target instanceof HTMLDocument) return;
    if (e.target.closest(".bSel")) return;
    for (let i = 0; i < registered.length; i++)
      clickSelect(registered[i], -1);
  });
  document.addEventListener("keydown", (e) => {
    if (!e.target.classList.contains("betterSelect") && !e.target.classList.contains("option")) return;
    let inp;
    if (!e.target.classList.contains("betterSelect"))
